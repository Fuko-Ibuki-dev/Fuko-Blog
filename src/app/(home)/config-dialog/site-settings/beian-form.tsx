...
export function BeianForm({ formData, setFormData }: BeianFormProps) {
	return (
		<div className='space-y-2'>
			<label className='mb-2 block text-sm font-medium'>Filing Information</label>
			<div className='grid grid-cols-2 gap-2'>
				<div>
					<label className='mb-1 block text-xs text-gray-600'>Filing ID</label>
					<input
						type='text'
						value={formData.beian?.text || ''}
						onChange={e => setFormData({ ...formData, beian: { ...(formData.beian || { text: '', link: '' }), text: e.target.value } })}
						placeholder='e.g., ICP No. 12345678'
						className='bg-secondary/10 w-full rounded-lg border px-4 py-2 text-sm'
					/>
				</div>
				<div>
					<label className='mb-1 block text-xs text-gray-600'>Filing link (optional)</label>
					<input
						type='url'
